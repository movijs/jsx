                ...
                if (t.isStringLiteral(attr.key)) {
                    if (attr.key.value.toLowerCase() === 'class' || attr.key.value.toLowerCase() === 'classname') {
                        ...
                        if (t.isObjectExpression(attr.value)) {
                            ...
                            //totalBindClass.push(attr.value)
                            //totalBindClass.push(t.arrowFunctionExpression([], t.blockStatement([t.returnStatement(attr.value)])));
...
const ParentIfStatement = (path: t.Expression | t.BlockStatement, state: any[]): any[] => {
    ...
    if (t.isFunctionExpression(path)) {
        ...
        //state.push(path.object)
        //state = ParentIfStatement(path., state);

    } else if (t.isIdentifier(path)) {

        // state.splice(0, 0, path)
    } else if (t.isCallExpression(path)) {

        //state.splice(0, 0, path.callee)
...
export const returnIfStatement = (value: any, enableDefaultreturn: boolean = true, defaultLiteral: any = t.booleanLiteral(false)): any => {
    ...
    if (t.isLogicalExpression(value)) {
        ...
        rtrn = t.returnStatement(value);// t.returnStatement(t.conditionalExpression(mx, t.callExpression(value, []), value));
        ...
        // if (enableDefaultreturn) { 
        //     return t.arrowFunctionExpression([], t.blockStatement([value,t.returnStatement(defaultLiteral)]));
        // } else { 
        //     return t.arrowFunctionExpression([], t.blockStatement([value]));
        // } 
        return value;
    }
...
export const findIfStatements = (value: t.Expression): any => {
    ...
    const setRight = (l: t.Expression, x: t.Expression, current: number) => {
        if (t.isLogicalExpression(l) && t.isNullLiteral(l.right)) {
            l.right = x;
            // l.right = t.logicalExpression('&&',t.binaryExpression('!==',(x as t.LogicalExpression).left,t.nullLiteral()),t.binaryExpression('!==',(x as t.LogicalExpression).left,t.identifier('undefined'))) ; 
            // l.right = t.logicalExpression('&&',t.binaryExpression('!==',(x as t.LogicalExpression).left,t.nullLiteral()),t.binaryExpression('!==',(x as t.LogicalExpression).left,t.identifier('undefined'))) ; 
        } else {
    ...
    if (ifs.length > 1) {
        ifs.forEach((ttt, index) => {
            if (!t.isArrowFunctionExpression(ttt) || !t.isFunctionExpression(ttt)) {
                ...
                //ifStatement =t.logicalExpression('&&',t.binaryExpression('!==',(ttt as t.LogicalExpression).left,t.nullLiteral()),t.binaryExpression('!==',(ttt as t.LogicalExpression).left,t.identifier('undefined'))) ; 
                ifStatement = ttt;
            }
        })
    } else {
